...
const handleSign = (e)=>{
    ...
    if (existingEmail) {
        ...
        errorM.textContent = "Las contrase\xf1as no coinciden";
        errorContainer.style.display = "block";
    }
};
const handleLogin = (e)=>{
    ...
    if (!user) {
        ...
        // show error message: password missmatch
        errorM.textContent = "Contrase\xf1a incorrecta";
        errorContainer.style.display = "block";
    }
};
...
const initializeLocalStorage = ()=>{
    ...
    if (users === null) window.localStorage.setItem("users", JSON.stringify([]));
    if (user === null) window.localStorage.setItem("user", JSON.stringify({}));
};
const startLoading = ()=>{
    const user = getUser();
